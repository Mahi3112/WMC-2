...
const DonationPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchVirtualCurrency = async () => {
            try {
                ...
                const response = await axios.get('https://wmc-project-tk78.onrender.com/user/getprofile', {
                    headers: {
    ...
    const handleDonation = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await axios.post('https://wmc-project-tk78.onrender.com/donation/donate', { amount }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
